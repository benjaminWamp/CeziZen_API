...
export class ExerciseUserController {
  ...
  @Post()
  create(@Body() createExerciseUserDto: CreateExerciseUserDto) : Promise<ApiReturns<ExerciseUserType | null>> {
  ...
  @Get(':id')
  findOne(@Param('id', ParseIntPipe) id: number) : Promise<ApiReturns<ExerciseUserType | null>> {
    return this.exerciseUserService.findOne(id);
  }

  @Patch(':id')
  update(@Param('id', ParseIntPipe) id: number, @Body() updateExerciseUserDto: UpdateExerciseUserDto) : Promise<ApiReturns<ExerciseUserType>>  {
